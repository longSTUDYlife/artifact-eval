...
\begin{itemize}
  ...
  \item HAR checkpoints: \path{Figure14d/checkpoints/}
        (RD / RD+RA / RD+RA+RE file-split weights).
  \item HAR test maps: \path{Figure14d/filtered/4.mat}
        (CPU eval). Files \texttt{1.mat}--\texttt{6.mat} are in the repo
        for optional retrain (seed~42; train 1,2,5,6; val~3; test~4).
  \item HAR packed confusion matrices: \path{Figure14d/cms/}.
\end{itemize}

\subsubsection{Models}

HAR uses a CNN+LSTM. Fig.~14(d) is reproduced by loading the packed
checkpoints and running test inference on \texttt{4.mat} (CPU;
\texttt{--eval}). Retraining is optional and requires a local CUDA rebuild.
...
Fig.~14(d) runs on this CPU image (\texttt{--eval}). A CUDA rebuild is
needed only if you want to retrain HAR.
...
Fig.~14(d)    & \figflag{14d}  & HAR CMs (CPU \texttt{--eval}) \\
\bottomrule
\end{tabularx}
}
...
\figflag{14d}  & 2--8\,min CPU eval of packed checkpoints \\
...
\vspace{0.35em}
Fig.~14(d) from packed checkpoints on \texttt{4.mat} matches the paper
(CPU \texttt{--eval}). Optional retrain: local CUDA rebuild, same file
split, preserves RD $<$ RD+RA $<$ RD+RA+RE. Figure labels:
...
To retrain HAR, rebuild a CUDA image locally
(\texttt{docker build --build-arg USE_CUDA=1}; include all six
\texttt{filtered/*.mat} in the image) and set
\texttt{FIGURE14D\_TRAIN=1}.
