...
\begin{description}
  ...
  \item[Data set]
    Packed CIR (\texttt{.npz}, $\approx$651\,MB) and six HAR
    \texttt{.mat} files ($\approx$2.0\,GB).
  \item[Environment]
    Docker. MATLAB is not required.
  \item[Hardware]
    4+ CPU cores; 8\,GB RAM minimum, 16\,GB recommended;
    $\approx$12\,GB disk. GPU optional (Fig.~14(d) retrain only).
  \item[Execution]
...
The review is best-effort double-blind. The anonymous artifact URL is
included with this HotCRP submission:

\begin{center}
\small\texttt{<ANONYMOUS-ARTIFACT-URL>}
\end{center}

After evaluation, the same snapshot will receive a public DOI.
...
docker run --rm -it \
  ...
  done-figures-artifact
\end{lstlisting}

The container prints a figure catalog. The first image build downloads
PyTorch (about 10--30 minutes). A CUDA image is $\approx$8\,GB; the CPU
image is smaller.

Optional GPU image (Linux amd64):

\begin{lstlisting}
docker build --platform linux/amd64 \
  --build-arg USE_CUDA=1 \
  -t done-figures-artifact .
\end{lstlisting}
...
Fig.~13(a)(b) & \figflag{13a}  & Env-1 scatter; Env-1 localization error \\
Fig.~13(e)(f) & \figflag{13e}  & Env-1 sensing trajectory and error \\
