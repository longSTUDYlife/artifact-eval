...
const byte DW1000Class::MODE_IN_CIRCUIT_2[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_256, CHANNEL_1};
...
byte cirData[CIR_DATA_LENGTH_100];
byte cirData_2[CIR_DATA_LENGTH_100];
float g_lastRange = 0.0f;
dwt_rxdiag_t *diag = (dwt_rxdiag_t*)malloc(sizeof(dwt_rxdiag_t));
dwt_rxdiag_t *diag_2 = (dwt_rxdiag_t*)malloc(sizeof(dwt_rxdiag_t));

volatile bool DW1000Class::_dmaBusy = false;
volatile bool DW1000Class::_dmaError = false;
volatile bool DW1000Class::_dmaDataReady = false;
byte* DW1000Class::_dmaRxBuffer = NULL;
unsigned int DW1000Class::_dmaLength = 0;
//void (*DW1000Class::_dmaCallback)(byte*, unsigned int) = NULL;

unsigned int g_dw1000_ss_pin = 0;
SPIClass* g_dw1000_spi = NULL;
volatile bool g_dmaBusy = false;
volatile bool g_dmaError = false;
volatile bool g_dmaDataReady = false;  // ✅ 标记DMA数据已准备好（轮询或中断完成）
volatile bool g_dmaHandlesCleanup = false;  // ✅ DMA中断会处理清理工作
byte* g_dmaRxBuffer = NULL;
unsigned int g_dmaLength = 0;
//void (*g_dmaCallback)(byte*, unsigned int) = NULL;
...
void DW1000Class::commitConfiguration() {
	...
	writeValueToBytes(antennaDelayBytes, 16405, LEN_STAMP);//16405 for tag, 16485 for anchor
...
void DW1000Class::setXtalTrim5bit(uint8_t trim5){
	trim5 &= 0x1F;
	byte v = (byte)(0x60 | trim5);
	writeBytes(FS_CTRL, FS_XTALT_SUB, &v, 1);
}

uint8_t DW1000Class::getXtalTrim5bit(){
	byte v; readBytes(FS_CTRL, FS_XTALT_SUB, &v, 1);
	return (uint8_t)(v & 0x1F);
}

uint8_t DW1000Class::servoXtalByPpm(float ppm, uint8_t currentTrim,
                                    float deadband_ppm /*=0.8f*/) {
    const float SLOPE_PPM_PER_STEP = 0.83f;   // 每+1步，ppm 的变化量（负值）
    const int   MAX_STEP_ONCE      = 3;        // 每次最多跳几步，防止过冲

    // 已在死区内则不动
    if (fabsf(ppm) <= deadband_ppm) return currentTrim;

    // 目标：令 new_ppm ≈ ppm + SLOPE * dtrim ≈ 0
    // => dtrim ≈ -ppm / SLOPE
    float dtrim_f = -ppm / SLOPE_PPM_PER_STEP;

    // // 去掉死区内那部分的修正量，避免来回抖
    // if (dtrim_f > 0) dtrim_f -= deadband_ppm / fabsf(SLOPE_PPM_PER_STEP);
    // else             dtrim_f += deadband_ppm / fabsf(SLOPE_PPM_PER_STEP);

    int dtrim = (int)roundf(dtrim_f);
    // if (dtrim == 0) dtrim = (ppm > 0) ? +1 : -1;           // 至少走一步
    if (dtrim >  MAX_STEP_ONCE)  dtrim =  MAX_STEP_ONCE;   // 限幅
    if (dtrim < -MAX_STEP_ONCE)  dtrim = -MAX_STEP_ONCE;

    int v = (int)currentTrim + dtrim;
    if (v < 0)  v = 0;
    if (v > 31) v = 31;

    setXtalTrim5bit((uint8_t)v);    // 内部务必保证 bits7:5 = 0b011
    return (uint8_t)v;
}

float DW1000Class::cfo_ppm_via_formula(int32_t cint) {
    const float Fs = 998.4e6f;
    //const float N  = (dataRate == TRX_RATE_110KBPS) ? 8192.0f : 1024.0f;
	const float N  = 1024.0f;
    float Fc = 3494.4e6f;                         // 默认 ch5
    // if (channel == CHANNEL_2) Fc = 3993.6e6f;
    // else if (channel == CHANNEL_3) Fc = 4492.8e6f;
    // Hz
	//Serial.print("cint="); Serial.print(cint);
    const float Foff = (float)cint * (1.0f / (1 << 17)) * Fs /(2.0f * N );
    // ppm
    return -1.0e6f * (Foff / Fc);
}

int32_t DW1000Class::getCarrierIntegratorRaw() {
    byte b[LEN_CAR_INT];
    readBytes(DRX_TUNE, DRX_CAR_INT_SUB, b, LEN_CAR_INT);   // 低字节在前
    uint32_t v = (uint32_t)b[0] | ((uint32_t)b[1] << 8) | ((uint32_t)b[2] << 16);
    v &= 0x001FFFFF;                    // 21 bit
    if (v & 0x00100000) v |= 0xFFE00000; // 符号扩展到 32-bit
    return (int32_t)v;
}


void DW1000Class::setClockAcc() {
		byte pmscctrl0[LEN_PMSC_CTRL0];
        readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0); 
		pmscctrl0[0] |= 0x48;
		pmscctrl0[1] |= 0x80;
...
void DW1000Class::resetClockAcc(){
		byte pmscctrl0[LEN_PMSC_CTRL0];
        readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0); 
		pmscctrl0[0] &= 0xB3;
		// pmscctrl0[0] |= 0x48;
...
void DW1000Class::readAccumulatorData(byte* buffer, uint16_t length) {
	readBytesDMA(ACC_MEM, 700*4+1, buffer, length);

	//readBytesDMA_Async(ACC_MEM, 1, buffer, length, NULL);
}

void DW1000Class::readAccumulatorDataMulti(word offset1, uint16_t len1,
                                             word offset2, uint16_t len2,
                                             byte* buffer) {
    // 读取第一段
    readBytesDMA(ACC_MEM, offset1, buffer, len1);
    
    // 读取第二段，拼接到第一段后面
    readBytesDMA(ACC_MEM, offset2, buffer + len1, len2);
}



uint16_t DW1000Class::computeCRC16(const byte* data, uint16_t length) {
...
void DW1000Class::GetDiagnosticInfo(dwt_rxdiag_t *rxdiag, byte* cirdataBuffer) {
	byte noiseBytes[LEN_STD_NOISE];
	byte fpAmpl1Bytes[LEN_FP_AMPL1];
	byte fpAmpl2Bytes[LEN_FP_AMPL2];
	byte fpAmpl3Bytes[LEN_FP_AMPL3];
	byte fpIndexBytes[LEN_FP_INDEX];
	byte maxNoiseBytes[LEN_LDE_THRESH];
	byte rxFrameInfo[LEN_RX_FINFO];
	readBytes(RX_TIME, FP_AMPL1_SUB, fpAmpl1Bytes, LEN_FP_AMPL1);
	readBytes(RX_FQUAL, FP_AMPL2_SUB, fpAmpl2Bytes, LEN_FP_AMPL2);
	readBytes(RX_FQUAL, FP_AMPL3_SUB, fpAmpl3Bytes, LEN_FP_AMPL3);
	...
	readBytes(LDE_IF, LDE_THRESH_SUB, maxNoiseBytes, LEN_LDE_THRESH);
	readBytes(RX_FINFO, 0x00, rxFrameInfo, 4);
	rxdiag->stdNoise = (unsigned int)noiseBytes[0] | ((unsigned int)noiseBytes[1] << 8);
	//rxdiag->firstPathAmp1 = (unsigned int)fpAmpl1Bytes[0] | ((unsigned int)fpAmpl1Bytes[1] << 8);
	rxdiag ->firstPathAmp1 = (uint16_t)(g_lastRange * 100);
	rxdiag->firstPathAmp2 = (unsigned int)fpAmpl2Bytes[0] | ((unsigned int)fpAmpl2Bytes[1] << 8);
	rxdiag->firstPathAmp3 = (unsigned int)fpAmpl3Bytes[0] | ((unsigned int)fpAmpl3Bytes[1] << 8);
	rxdiag->firstPath = ((unsigned int)fpIndexBytes[0] | ((unsigned int)fpIndexBytes[1] << 8));
	rxdiag->maxNoise = (unsigned int)maxNoiseBytes[0] | ((unsigned int)maxNoiseBytes[1] << 8);
	uint32_t rxFinfoVal = (unsigned int)rxFrameInfo[0] 
                    | (unsigned int)(rxFrameInfo[1] << 8) 
                    | (unsigned int)(rxFrameInfo[2] << 16) 
                    | (unsigned int)(rxFrameInfo[3] << 24);
	rxdiag->rxPreamCount = (rxFinfoVal >> 20) & 0x0FFF;
	// for(int i= 0; i<CIR_DATA_LENGTH_100; i++) { 
	// 	//rxdiag->cir_data[i] = cirData[729*4+1 + i];
	// 	rxdiag->cir_data[i] = cirData[i]; // 400 samples of CIR data, 4 bytes each
	// }
	memcpy(rxdiag->cir_data, cirdataBuffer, CIR_DATA_LENGTH_100);  // 一次性拷贝400字节

}

void DW1000Class::sendPacket(uint8_t seq, byte* src, dwt_rxdiag_t* diagPtr) {
	PacketHeader header;
    header.start = PACKET_START;
    //header.packetType = 0x01;  
    // static uint8_t seq = 0;
    // header.sequence = seq++;
	header.packetType = src[0];  // use the first byte of src as packetType
	header.sequence = seq;

	uint16_t diagSize = sizeof(dwt_rxdiag_t);
    uint16_t payloadLength = diagSize;
    header.length = payloadLength;

	uint16_t packetSize = sizeof(PacketHeader) + payloadLength + sizeof(PacketFooter);
    ...
    if(packet == NULL) {
	 ...
	 memcpy(packet + sizeof(PacketHeader), diagPtr, diagSize);

	 uint16_t crc = computeCRC16(packet, sizeof(PacketHeader) + payloadLength);

	 PacketFooter footer;
	 footer.checksum = crc;
	 footer.end = PACKET_END;

	 memcpy(packet + sizeof(PacketHeader) + payloadLength, &footer, sizeof(PacketFooter));
    ...
    //             uint32_t e = lastElapsed_1;
    //             gotElapsed_1 = false;

    //             Serial.print("sendpacket 耗时：");
    //             Serial.print(e);
    //             Serial.println(" us");
    //         }
...
DW1000Time DW1000Class::setDelayAbsolute(const DW1000Time& absTime) {
    // 选择 TX 或 RX 的 delayed 模式位
    if(_deviceMode == TX_MODE) {
        setBit(_sysctrl, LEN_SYS_CTRL, TXDLYS_BIT, true);
    } else if(_deviceMode == RX_MODE) {
        setBit(_sysctrl, LEN_SYS_CTRL, RXDLYS_BIT, true);
    ...
    // 写 DX_TIME（40-bit，最低位必须清零）
    byte ts[5];
    DW1000Time t = absTime;           // 绝对时刻（芯片时钟域）
    t.getTimestamp(ts);
    ts[0] = 0;                        // LSB = 0
    ts[1] &= 0xFE;                    // bit0 必须为 0
    writeBytes(DX_TIME, NO_SUB, ts, LEN_DX_TIME);

    // 返回“期望 TX/RX 绝对时刻 + 天线延时”供上层记账（与 setDelay 行为一致）
    t.setTimestamp(ts);
    t += _antennaDelay;
    return t;
}

// void DW1000Class::readBytesDMA(byte cmd, word offset, byte data[], unsigned int n) {
//     byte header[3];
//     int headerLen = 1;
//     int i;
...
void DW1000Class::readBytesDMA(byte cmd, word offset, byte data[], unsigned int n) {
	...
	// 等待SPI空闲再配置DMA
	while(SPI_SR(SPI2_BASE) & SPI_SR_BSY);

	// 清空SPI RX缓冲区中的残留数据
	while(SPI_SR(SPI2_BASE) & SPI_SR_RXNE) {
		volatile uint8_t dummy __attribute__((unused)) = SPI_DR(SPI2_BASE);
	}

	// 使能DMA时钟
	rcc_enable_clock(RCC_DMA1);

	// spi_disable_dma(SPI2, SPI_DMA_TX_RX);

	// 禁用DMA通道（SPI2使用Channel 4和5）
	dma_disable(DMA_SPI2_RX);  // Channel 4
	dma_disable(DMA_SPI2_TX);  // Channel 5

	// 等待DMA停止（Channel 4和5的索引是3和4）
	uint32_t timeout = 1000;
	while((DMA_CCR(DMA1_BASE, 3) & DMA_CCR_EN) && timeout--);  // Ch4 idx=3
	timeout = 1000;
	while((DMA_CCR(DMA1_BASE, 4) & DMA_CCR_EN) && timeout--);  // Ch5 idx=4

	// 完全清除CCR寄存器，确保干净状态
	DMA_CCR(DMA1_BASE, 3) = 0;  // Ch4 RX
	DMA_CCR(DMA1_BASE, 4) = 0;  // Ch5 TX

	// 清除中断标志
	DMA_IFCR(DMA1_BASE) = DMA_IFCR_CGIF4 | DMA_IFCR_CGIF5;

	// 配置RX DMA（SPI2_RX = Channel 4）
	dma_setup_channel(
		DMA_SPI2_RX,
		(u32)data,
		(u32)&SPI_DR(SPI2_BASE),
		n,
		DMA_CCR_MINC | DMA_CCR_PL_HIGH 
	);

	// 配置TX DMA（SPI2_TX = Channel 5）
	static byte dummyTx = JUNK;
	dma_setup_channel(
		DMA_SPI2_TX,
		(u32)&dummyTx,
		(u32)&SPI_DR(SPI2_BASE),
		n,
		DMA_CCR_PL_MEDIUM | DMA_CCR_DIR
	);

	// 确保SPI2的DMA请求已使能（第一次需要使能，后续保持不变）
	if((SPI_CR2(SPI2_BASE) & (SPI_CR2_TXDMAEN | SPI_CR2_RXDMAEN)) == 0) {
		// 只有第一次需要设置，禁用SPI后设置
		uint32_t cr1_backup = SPI_CR1(SPI2_BASE);
		SPI_CR1(SPI2_BASE) = cr1_backup & ~SPI_CR1_SPE;
		spi_enable_dma(SPI2, SPI_DMA_TX_RX);
		SPI_CR1(SPI2_BASE) = cr1_backup;
	}

	// 启动DMA传输 - 关键：必须先启动RX再启动TX！
	dma_enable(DMA_SPI2_RX);  // RX先启动，准备接收
	dma_enable(DMA_SPI2_TX);  // TX后启动，产生时钟

		uint32_t loopCount = 0;
		timeout = 100000;
		while((DMA_CNDTR(DMA1_BASE, 3) > 0) && timeout--) {
			loopCount++;
		}

		// 等待SPI完全空闲
		timeout = 10000;
		while((SPI_SR(SPI2_BASE) & SPI_SR_BSY) && timeout--);

		// 手动清理：禁用DMA通道
		dma_disable(DMA_SPI2_RX);
		dma_disable(DMA_SPI2_TX);

		// 清除DMA中断标志
		DMA_IFCR(DMA1_BASE) = DMA_IFCR_CGIF4 | DMA_IFCR_CGIF5;

		// 清除NVIC pending位
		NVIC_ICPR(0) = (1 << DMA_SPI2_RX_IRQ);
