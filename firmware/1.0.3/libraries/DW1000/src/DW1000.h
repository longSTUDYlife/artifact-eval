...
#define ACC_MEM 0x25
#define CIR_DATA_LENGTH 400
#define CIR_DATA_LENGTH_100 400
#define CIR_DATA_LENGTH_300 400

#define LEN_FP_INDEX 2
#define LEN_LDE_THRESH 2
#define FP_INDEX_SUB 0x05
#define LDE_THRESH_SUB 0x0000
#define PACKET_START 0xAA
#define PACKET_END   0x55

#define LEN_CAR_INT     3
...
extern byte cirData[CIR_DATA_LENGTH_100];
extern byte cirData_2[CIR_DATA_LENGTH_100];
extern float g_lastRange;

typedef struct {
    uint8_t start;       // PACKET_START 0xAA
    uint16_t length;     // packet length in bytes
    uint8_t packetType;  // 0x01 diag
    uint8_t sequence;    // 1,2,3... sequence number of the packet
} PacketHeader;
...
typedef struct
{
	...
	byte          cir_data[CIR_DATA_LENGTH_100]; // CIR data, only for 100 samples, 400 bytes
	// Note: cir_data is not part of the struct size, it is a buffer to hold CIR data
	// It should be filled in the readDiagnosticData() function
	// and should be freed by the caller if necessary.
	// The length of cir_data is defined by CIR_DATA_LENGTH_100
	// so it can be used to store 100 samples of CIR data.
}dwt_rxdiag_t ;
...
extern dwt_rxdiag_t *diag;
extern dwt_rxdiag_t *diag_2;
...
public:
	...
	*/
	...
	static uint8_t getXtalTrim5bit(); 
	static void    setXtalTrim5bit(uint8_t trim5);     // 写 FS_XTALT（bits7:5 强制 011）
	static uint8_t servoXtalByPpm(float ppm, uint8_t currentTrim, float deadband_ppm=0.8f);
	static float cfo_ppm_via_formula(int32_t cint);
	static int32_t getCarrierIntegratorRaw();   // 读 0x27:0x28，返回带符号 21-bit

	static void readAccumulatorData(byte* buffer, uint16_t length);
	static void setClockAcc();
	static void resetClockAcc();
	void readAccumulatorDataMulti(word offset1, uint16_t len1,
                               word offset2, uint16_t len2,
                               byte* buffer);


	static void sendPacket(uint8_t seq, byte* source, dwt_rxdiag_t *diagPtr);
	static void GetDiagnosticInfo(dwt_rxdiag_t *rxdiag, byte* cirdataBuffer);
	static uint16_t computeCRC16(const byte* data, uint16_t length);
	
	DW1000Time setDelayAbsolute(const DW1000Time& absTime);
	...
	static void readBytesDMA(byte cmd, word offset, byte data[], unsigned int n);
	static void dmaAsyncCleanup();

private:

	static volatile bool _dmaBusy;
	static volatile bool _dmaError;
	static volatile bool _dmaDataReady;
	static byte* _dmaRxBuffer;
	static unsigned int _dmaLength;
	//static void (*_dmaCallback)(byte*, unsigned int);
	static bool _dmaInitialized;
