...
void dma_channel_reset(dma_channel_t dma)
{
	u32 base;
	int channel;
	...
	DMA_CCR(base, channel) = 0;
	
	/* Reset data transfer number. */
	DMA_CNDTR(base, channel) = 0;
	
	/* Reset peripheral address. */
	DMA_CPAR(base, channel) = 0;
	
	/* Reset memory address. */
	DMA_CMAR(base, channel) = 0;
	
	/* Clear all interrupt flags for this channel. */
	DMA_IFCR(base) = (DMA_IFCR_CGIF1 | DMA_IFCR_CTCIF1 | 
	                   DMA_IFCR_CHTIF1 | DMA_IFCR_CTEIF1) << (4 * channel);
}