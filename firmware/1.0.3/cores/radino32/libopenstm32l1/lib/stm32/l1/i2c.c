...
int i2c_addr(i2c_t i2c, u8 sla)
{
  ...
  int i;
	...
	base = base_addr(i2c);
	I2C_DR(base) = sla;
  for (i = 0; i < I2C_TIMEOUT_DEFAULT; i++) {
	 if (!(!(I2C_SR1(base) & (I2C_SR1_ADDR | I2C_SR1_ERROR))))
    break;
  }
	if (I2C_SR1(base) & I2C_SR1_ERROR)
		return -I2C_ERROR_STATUS;
...
int i2c_write(i2c_t i2c, u8 *buf, int nbyte)
{
	...
	base = base_addr(i2c);
	for (i = 0; i < nbyte; i++) {
    for (j = 0; j < I2C_TIMEOUT_DEFAULT; j++) {
		  if(!(!(I2C_SR1(base) & (I2C_SR1_TXE | I2C_SR1_ERROR))))
			 break;
    }
		if (I2C_SR1(base) & I2C_SR1_ERROR)
			return -I2C_ERROR_STATUS;
	...
	for (i = 0; i < I2C_TIMEOUT_DEFAULT; i++) {
		  if(!(!(I2C_SR1(base) & (I2C_SR1_BTF | I2C_SR1_ERROR))))
			 break;
  }
	if (I2C_SR1(base) & I2C_SR1_ERROR)
		return -I2C_ERROR_STATUS;
...
int i2c_read(i2c_t i2c, u8 *buf, int nbyte)
{
	...
	for (i = 0; i < nbyte - 1; i++) {
    for (j = 0; j < I2C_TIMEOUT_DEFAULT; j++) {
      if(!(!(I2C_SR1(base) & (I2C_SR1_RXNE | I2C_SR1_ERROR))))
        break;
    }
		if (I2C_SR1(base) & I2C_SR1_ERROR)
			return -I2C_ERROR_STATUS;
