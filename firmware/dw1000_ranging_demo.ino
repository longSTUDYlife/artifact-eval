/*
 ...
 http://wiki.in-circuit.de/index.php5?title=dw1000_ranging_demo
*/
...
void setup() {
  ...
  if(PIN_LED_1!=0xFF) pinMode(PIN_LED_1,OUTPUT);
  if(PIN_LED_1!=0xFF) digitalWrite(PIN_LED_1,LOW);
  if(PIN_LED_2!=0xFF) pinMode(PIN_LED_2,OUTPUT);
  if(PIN_LED_2!=0xFF) digitalWrite(PIN_LED_2,LOW);
  ...
  if (weAreAnchor) {
    if(WE_ARE_LISTENER == 1){
      DW1000Ranging.startAsListener("7D:00:22:EA:82:60:3B:9C", DW1000.MODE_IN_CIRCUIT_2, MY_SHORT_ADDRESS);
      }else{
            DW1000Ranging.startAsTag("7D:00:22:EA:82:60:3B:9C", DW1000.MODE_IN_CIRCUIT_2, MY_SHORT_ADDRESS);
        }
    //Start the module as tag since the library switched naming.

  } else {
    //Start the module as anchor since the library switched naming.
    DW1000Ranging.startAsAnchor("7D:00:22:EA:82:60:3B:9C", DW1000.MODE_IN_CIRCUIT_2, MY_SHORT_ADDRESS);
