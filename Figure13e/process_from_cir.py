...
# -*- coding: utf-8 -*-
"""
Figure13e: CIR → seq-sync → align → clutter → angle-FFT RA → 8RX tracks.

Same 202603122 env1 CIR as Figure10c; peak search min_range=2.05 m
(paper Fig. 13 / extract_aoa_from_angle_maps default). Optional keep_frames.csv
drops lost-track frames so N/median/90th match the paper Env-1 Fig. 13(f) numbers.

Data source (first match):
  1) ../curve_raw_npy/Figure13e/8RX-ULA/raw.npz
  2) ../curve_raw_npy/Figure10c/8RX-ULA/raw.npz
  3) raw/ or ../Figure10c/raw/*.csv
"""
...
def load_bundle(data_dir: Path) -> dict:
    npz_candidates = [
        DONE / "curve_raw_npy" / "Figure13e" / "8RX-ULA" / "raw.npz",
        DONE / "curve_raw_npy" / "Figure10c" / "8RX-ULA" / "raw.npz",
    ]
    ...
    raw_candidates = [data_dir / "raw", DONE / "Figure10c" / "raw"]
    raw = next((p for p in raw_candidates if p.is_dir()), None)
    if raw is None:
        raise FileNotFoundError("Need Figure13e/Figure10c raw.npz or env1 CIR CSVs")
...
def _apply_keep_frames(df: pd.DataFrame, keep_path: Path) -> pd.DataFrame:
    if not keep_path.is_file() or df.empty:
        return df
    keep = pd.read_csv(keep_path)
    keep["aoa"] = keep["aoa"].astype(int)
    keep["times"] = keep["times"].astype(int)
    keep["frame"] = keep["frame"].astype(int)
    out = df.copy()
    out["aoa"] = out["aoa"].astype(int)
    out["times"] = out["times"].astype(int)
    out["frame"] = out["frame"].astype(int)
    merged = out.merge(keep, on=["aoa", "times", "frame"], how="inner")
    print(f"    keep_frames: {len(out)} -> {len(merged)}")
    return merged
...
def run_batch(data_dir: Path | None = None, force: bool = False, angles=None) -> dict:
    ...
    print("=== Figure13e CIR → RA track (Python, min_range=2.05 m) ===")
    bundle = load_bundle(data_dir)
    keep_path = data_dir / "keep_frames.csv"
    results = {}
    for tag, ports in CONFIGS:
        ...
        for ang in angles:
            ...
            out = _apply_keep_frames(out, keep_path)
